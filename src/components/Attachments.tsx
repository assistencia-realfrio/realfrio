...
const Attachments: React.FC<AttachmentsProps> = ({ orderId }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      showSuccess(`ARQUIVO '${stripUuidFromFile(newAttachment.name).toUpperCase()}' ANEXADO COM SUCESSO!`);
    } catch (error) {
  ...
  const handleDelete = async (attachmentId: string, attachmentName: string) => {
    ...
    try {
      ...
      showSuccess(`ANEXO '${stripUuidFromFile(attachmentName).toUpperCase()}' REMOVIDO.`);
    } catch (error) {
